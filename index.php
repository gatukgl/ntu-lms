 ...
 */
    ...
    require($CFG->dirroot . '/auth/manual/auth.php');

    ////////////////////////////// Token Listener /////////////////////////////
    global $DB; 
    if(!empty($_REQUEST['token'])){
        //echo $value_token;
        $value_token = $_REQUEST['token'];
        $observe_token_from_db = '';

        $tokenssql = "SELECT t.id, t.sid, t.token, t.validuntil, t.iprestriction, t.userid
                        FROM {external_tokens} t
                        ORDER BY t.timecreated ASC";

        $tokens = $DB->get_records_sql($tokenssql, array(EXTERNAL_TOKEN_PERMANENT));        
        if(!empty($tokens)){
